...
import 'package:tre_flutter/pages/test/test_page_7.dart';
import 'package:tre_flutter/pages/uxin/xin_detail_page.dart';
import 'package:tre_flutter/pages/uxin/xin_item_widget.dart';
...
class XinListPage extends StatefulWidget {
  HeaderWidgetBuild headerCreator;
  FooterWidgetBuild footerCreator;
  Widget footer2;

  XinListPage({Key key, this.headerCreator, this.footerCreator, this.footer2}) : super(key: key);

  @override
  _XinListPageState createState() => _XinListPageState(footer2: footer2);
}

class _XinListPageState extends State<XinListPage> {
  List<XinListModel> mModelList = [];
  ...
  _XinListPageState({this.footer2});
  ...
  Future<Null> _refreshData() async {
    ...
    var modelList = await _requestData();
    setState(() {
...
/// 网络请求
Future<List<XinListModel>> _requestData() async {
  Uri uri = XinListModel.getUri();
  ...
  try {
    ...
    log('_XinListPageState | _requestData | responseData = $responseData');
    return XinListModel.parseList(responseData);
  } catch (e) {
    ToastUtils.show("网络请求失败");
    log('_XinListPageState | _requestData | e.toString() = ' + e.toString());
    return new List();
  }
}
